...
// OCXO Vctl calibration: find an approximate value for Vctl
{
  ...
  //MAC adding initial messages to BT
  Serial2.println(F("Measure frequency for Vctl=1.5V"));
  Serial2.print(F("Set PWM Vctl to 1.5V, wait ")); Serial2.print(ocxo_calib_time); Serial2.println(F("s"));
  ...
  while (calib_countdown > 0)
    {
      ...
      //MAC adding initial messages to BT
      Serial2.print(calib_countdown); Serial.print(F("s "));
      delay(1000);    
    }  
  ...
  //MAC adding initial messages to BT
  Serial2.println();
  Serial2.print(F("f1 (average frequency for Vctl=1.5V): "));
  ...
  //MAC adding initial messages to BT
  Serial2.println(F("Measure frequency for Vctl=2.5V"));
  Serial2.println(F("Set PWM Vctl to 2.5V, wait ")); Serial2.print(ocxo_calib_time); Serial2.println(F("s"));
  ...
  while (calib_countdown > 0)
    {
      ...
      Serial.print(calib_countdown); Serial.print(F("s "));
      //MAC adding initial messages to BT
      Serial2.print(calib_countdown); Serial2.print(F("s "));
      delay(1000);    
    }  
  ...
  //MAC adding initial messages to BT
  Serial2.println();
  Serial2.print(F("f2 (average frequency for 2.5V Vctl): "));
  Serial2.print(f2,1);
  Serial2.println(F(" Hz"));
  Serial2.println();
  ...
  analogWrite(VctlPWMOutputPin, adjusted_PWM_output); 
  //MAC adding initial messages to BT
  Serial2.print(F("Calculated PWM: "));
  Serial2.println(adjusted_PWM_output);
 ...
 */
  ...
  Serialx.print(month);            // date mm/dd/yyyy
  Serialx.print(F("/"));
  Serialx.print(day);
  Serialx.print(F("/"));
  Serialx.print(year);
...
void setup()
{
  ...
  #endif // BLUETOOTH
  Serial2.begin(9600); // Hardware serial 2 to Bluetooth module
  ...
  //MAC
  Serial2.println();
  Serial2.println(F(Program_Name));
  Serial2.println(F(Program_Version));
  Serial2.println();
  Serial2.println(F("2Hz interrupt configured"));    
  Serial2.println(F("Serial interfaces configured"));  
  ...
  //MAC
  Serial2.println(F("Commands parser configured")); 
  ...
  // Setup LCD SPI ST7789 display
    ...
    //MAC
    Serial2.println(F("ST7789 LCD display configured"));
  #endif // LCD_ST7789
  ...
  // Setup TM1637 4-digit LED module
    ...
    //MAC
    Serial2.println(F("TM1637 4-digit LED clock display configured"));
  #endif // TM1637

  #ifdef GPSDO_UBX_CONFIG
    // Reconfigure the GPS receiver
    // first send the $PUBX configuration commands
    delay(9000); // give everything a moment to stabilize
    Serial.println("GPS checker program started");
    //MAC
    Serial2.println("GPS checker program started");
    ...
    //MAC
    Serial2.println(F("u-blox GPS module configured"));
  #endif // UBX_CONFIG
  ...
  // Setup AHT10 / AHT20 temperature and humidity sensor module
    ...
    //MAC
    Serial2.println(F("Testing for presence of AHT10 or AHT20 Sensor on I2C bus"));
    if (!aht.begin()) {
      Serial.println(F("Could not find AHT10 or AHT20 sensor, check wiring"));
      //MAC
      Serial2.println(F("Could not find AHT10 or AHT20 sensor, check wiring"));
      while (1) delay(10);
    }
    else Serial.println(F("AHTX0 sensor configured"));
    //MAC
    Serial2.println(F("AHTX0 sensor configured"));
  ...
  // Setup OLED I2C display
    ...
    //MAC
    Serial2.println(F("OLED display configured"));
  #endif // OLED 
  ...
  // Setup INA219 current sensor module
    ...
    if (! ina219.begin()) {
      ...
      //MAC
      Serial2.println(F("Could not find INA219 sensor, check wiring"));
      while (1) { delay(10); }
    }
    else Serial.println(F("INA219 sensor configured"));
    //MAC
    Serial2.println(F("INA219 sensor configured"));
  ...
  //MAC
  Serial2.println(F("16-bit PWM DAC configured"));

  #if (defined (GPSDO_BMP280_SPI) || defined (GPSDO_BMP280_I2C))
  // Initialize BMP280
  Serial.println(F("Testing for presence of BMP280 Sensor on I2C bus"));
  //MAC
  Serial2.println(F("Testing for presence of BMP280 Sensor on I2C bus"));
    ...
    if (!bmp.begin()) {
      ...
      //MAC
      Serial2.println(F("Could not find BMP280 sensor, check wiring"));
      while (1) delay(10);
    }
    else Serial.println(F("BMP280 sensor configured"));
    //MAC
    Serial2.println(F("BMP280 sensor configured"));
  ...
  //MAC
  Serial2.println();
  Serial2.println(F("GPSDO Starting"));
  Serial2.println();
